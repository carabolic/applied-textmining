...
import edu.stanford.nlp.ling.CoreAnnotations.ParaPosAnnotation;
...
import edu.stanford.nlp.ling.CoreAnnotations.SentencePosAnnotation;
...
import edu.stanford.nlp.ling.CoreAnnotations.WordPosAnnotation;
  ...
  protected Collection<String> featuresC(PaddedList<IN> cInfo, int loc) {
    ...
    if (flags.useInternal && flags.useExternal ) {
      ...
      if (flags.useNext) {
        ...
        if (flags.useDistSim) {
          // TODO: this is previous, should be next
          featuresC.add(p.get(DistSimAnnotation.class) + "-NDISTSIM");
        }
        if (flags.useIsURL) {
      ...
      if (flags.useMUCFeatures) {
        ...
        featuresC.add(c.get(WordPosAnnotation.class)+"-WORD_POSITION");
        featuresC.add(c.get(SentencePosAnnotation.class)+"-SENT_POSITION");
        featuresC.add(c.get(ParaPosAnnotation.class)+"-PARA_POSITION");
        featuresC.add(c.get(WordPosAnnotation.class)+ '-' +c.get(ShapeAnnotation.class)+"-WORD_POSITION_SHAPE");
      }
