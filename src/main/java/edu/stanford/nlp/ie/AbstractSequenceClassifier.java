...
public abstract class AbstractSequenceClassifier<IN extends CoreMap> implements Function<String, String> {
  ...
  public DocumentReaderAndWriter<IN> readerAndWriter; // = null;
  ...
  /**
   * Classify a List of tokens using a TrueCasingDocumentReader. <i>Note:</i>
   * This was fairly quickly added to build a Truecaser. It may be revised or
   * disappear.
   *
   * @param sentence
   ...
   */
  public List<IN> classifyWithCasing(List<IN> sentence) {
    List<IN> document = new ArrayList<IN>();
    int i = 0;
    for (IN word : sentence) {
      IN wi = tokenFactory.makeToken();
      if (readerAndWriter instanceof TrueCasingDocumentReaderAndWriter) {
        String wordStr = word.get(TextAnnotation.class);
        wi.set(TextAnnotation.class, wordStr.toLowerCase());

        if (flags.useUnknown) {
          wi.set(UnknownAnnotation.class,
              (TrueCasingDocumentReaderAndWriter.known(wi.get(TextAnnotation.class)) ? "false" : "true"));
          // System.err.println(wi.word()+" : "+wi.get("unknown"));
        }
      } else {
        wi.set(TextAnnotation.class, word.get(TextAnnotation.class));
      }
      wi.set(PositionAnnotation.class, Integer.toString(i));
      wi.set(AnswerAnnotation.class, backgroundSymbol());
      document.add(wi);
    ...
    for (IN wi : document) {
      ...
      if (flags.readerAndWriter.equalsIgnoreCase("edu.stanford.nlp.sequences.TrueCasingDocumentReader")) {
        ...
        if (wi.get(AnswerAnnotation.class).equals("INIT_UPPER")
            || wi.get(PositionAnnotation.class).equals(flags.backgroundSymbol)) {
        ...
        word.set(NamedEntityTagAnnotation.class, wi.get(AnswerAnnotation.class));
      }
      i++;
    }
    return sentence;
  ...
  public List<List<IN>> classify(String str) {
    DocumentReaderAndWriter<IN> oldRW = readerAndWriter;
    readerAndWriter = new PlainTextDocumentReaderAndWriter<IN>();
    readerAndWriter.init(flags);
   ...
   *
   * @param sentences
   *          The String to be classified. It will be tokenized and divided into
   *          documents according to (heuristically determined) sentence
   *          boundaries.
   * @param outputFormat
  ...
  public String classifyToString(String sentences, String outputFormat, boolean preserveSpacing) {
    ...
    DocumentReaderAndWriter<IN> tmp = readerAndWriter;
    readerAndWriter = new PlainTextDocumentReaderAndWriter<IN>();
    readerAndWriter.init(flags);
    ...
    for (List<IN> doc : documents) {
      ...
      sb.append(((PlainTextDocumentReaderAndWriter<IN>) readerAndWriter).getAnswers(docOutput, outFormat,
          preserveSpacing));
    }
    readerAndWriter = tmp;
    return sb.toString();
  ...
  public List<String> segmentString(String sentence) {
    ...
    //
    // TODO: use printAnswers(List<IN> doc, PrintWriter pw)
    // instead
    //
   ...
   *         answers (with <code>setAnswer()</code>).
   */
  ...
  /**
   * Trains a classifier from a Collection of sequences Note that the Collection
   * can be (and usually is) an ObjectBank.
   *
   * @param docs
  ...
  public void classifyAndWriteAnswers(String baseDir, String filePattern) throws Exception {
    ObjectBank<List<IN>> documents = makeObjectBankFromFiles(baseDir, filePattern);
    classifyAndWriteAnswers(documents);
  }
  ...
  private void classifyAndWriteAnswers(ObjectBank<List<IN>> documents) throws Exception {
    classifyAndWriteAnswers(documents, System.out);
  }
